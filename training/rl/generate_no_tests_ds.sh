...
echo "NUM_GPUS=$NUM_GPUS DATASET_SIZE=$DATASET_SIZE MAX_ITEMS=$MAX_ITEMS"
echo "Preset values:"
...
OUTDIR="./coderm/dataset/taco_responses_${NUM_COMPS}_og_chunked_${G_START_IDX}_${G_END_IDX}/"
mkdir -p $OUTDIR
...
for ((i=0; i<NUM_GPUS; i++)); do
  ...
  CUDA_VISIBLE_DEVICES=$i python3 coderm/eval/taco_eval.py \
    --dataset "codegenning/taco_prompts" \
    --split train \
